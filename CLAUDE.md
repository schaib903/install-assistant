...
- **`01_install-windows.ps1` self-elevates** at startup via `Start-Process -Verb RunAs` if not already running as Administrator (single UAC prompt, wrapped in try/catch — declining just continues unelevated rather than aborting). This exists so machine-scope installs (e.g. Git) don't each trigger their own separate UAC prompt mid-run. `install-linux.sh` has no equivalent; it already gates on `sudo -v` once up front before installing anything.
- **`$FirefoxID` is resolved dynamically** in `01_install-windows.ps1` via `(Get-UICulture).TwoLetterISOLanguageName` — winget's `Mozilla.Firefox` package is hardcoded to the en-US build, and Mozilla ships per-locale IDs instead of a runtime language switch, so a German Windows display language swaps in `Mozilla.Firefox.de`. Every other catalog entry uses a language-neutral/MUI installer that already follows the OS display language on its own, so this special-case only exists for Firefox — don't generalize it to other programs without first checking (via `winget show --id <ID>`) whether they actually need it.
...
3. **Stable copy before registering** — `Registriere-Aufgabe` first copies the running script to `%LOCALAPPDATA%\install-assistant\02_update-windows.ps1` (`$StabilesSkript`) and points the scheduled task's action at *that* path, not at `$PSCommandPath`. Reason: the script is normally run from a cloned repo folder the user could later delete or move (e.g. after a bootstrap-kit cleanup), which would otherwise silently break the task. `LOCALAPPDATA` needs no elevation to write to (elevation is only needed for `Register-ScheduledTask -RunLevel Highest` itself, see point 5). Re-running `-Register` overwrites the copy, so it's how you deploy an updated version of the script to the scheduled task.
4. **Weekly self-registration** — after a manual run, if no Scheduled Task named `"install-assistant - Weekly Program Update"` exists yet, it asks once (Y/N) whether to register one; `-Register` does the same non-interactively, `-Unregister` unregisters it *and* deletes `$StabilesSkript`, so nothing orphaned is left behind. The registered task calls the stable copy with `-Silent`, which suppresses the `Read-Host` calls (final pause + the registration question itself) so the unattended run never blocks on missing input. Unlike the internal function/variable names, these parameter names are part of the script's CLI interface (something a user actually types), so they were translated to English along with the rest of the visible surface.
...
6. **`-Status`** — read-only inspection of the registered task: state, the literal script path baked into the task's action (extracted from `$task.Actions.Arguments` via a `-File "..."` regex match, since `Register-ScheduledTask` doesn't store the path anywhere more structured — this will normally be `$StabilesSkript`, not the repo path), a `Test-Path` check on that path (defensive: the stable copy could still be deleted by hand or wiped by an AppData cleanup tool), and `Get-ScheduledTaskInfo`'s last/next run time and last result code.

## Architecture — `03_move-user-folders-windows.ps1`

A separate flow from the bootstrap script, standalone (no dependency on the bootstrap having run), for relocating the official Windows user folders — Desktop, Documents, Downloads, Pictures, Music, Videos, Favorites — to another drive. Built the same way (status/plan → single confirmation → execution → report).

**Folder set is deliberately not "all known folders"** — it's exactly the ones that appear in `HKCU:\...\Explorer\User Shell Folders` on a stock Windows profile (verified live against the registry, not assumed from memory). OneDrive, Saved Games, Searches, Contacts, and Links are excluded: the first is a user choice (moving it fights OneDrive's own sync), the other four have **no entry at all** in `User Shell Folders` on a default profile — they're managed via the newer `SHSetKnownFolderPath` API instead of a redirectable registry string, so writing a value for them the same way this script does for the other seven would very likely be silently ignored by Explorer rather than actually relocate anything. Don't add them to `$OrdnerDefinitionen` without implementing that different (P/Invoke-based) mechanism first.
...
2. **Folder-move planning** — always asked (Y/N). If yes: prompts for a target base folder, resolves the *current* path of each known folder dynamically via `shell:Desktop` / `shell:Personal` / `shell:Downloads` / `shell:My Pictures` / `shell:My Music` / `shell:My Video` / `shell:Favorites` (COM `Shell.Application`, not hardcoded paths — this is locale-proof, since folder names are localized on non-English Windows but the `shell:` alias never is), computes folder sizes, checks free space on the target drive (with a 10% margin) before adding anything to the plan, and warns + asks per-folder (y/N) before proceeding in two cases: the folder currently lives under a path containing `OneDrive` (moving a OneDrive-managed folder via registry edit can fight with OneDrive's own Known Folder Move feature), or — `elseif`, so it's not double-prompted for the same folder — the folder's current path doesn't match its standard `%USERPROFILE%\<Name>` location (`$def.StandardName`), meaning it was already redirected/migrated/symlinked before and a second move might not be what the user expects. Note that `ShellName`/`RegName` values (`"My Pictures"`, `"Personal"`, `"Downloads"`, etc.) are fixed Windows API identifiers, not translatable UI text — only the `Label` values (shown to the user) are in English.
